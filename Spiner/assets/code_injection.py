#!/usr/bin/env python3
# code_injection.py - Code Injection Scanner
...
PAYLOAD_FILE = "assets/payloads/Code Injection.txt"
TIMEOUT = 10

PHP_ERRORS = [
    "Warning:", "Parse error", "Fatal error",
    "system()", "exec()", "eval()",
...
def load_payloads() -> List[str]:
    try:
        ...
        return ["system('id');", "eval('system(\"id\")');"]

def detect_code_injection(response_text: str) -> bool:
    for pattern in PHP_ERRORS:
...
def run(target: str, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        for param in params:
            for payload in payloads[:30]:
                ...
                try:
                    ...
                    if detect_code_injection(resp.text):
                        vulnerable.append({
                            "param": param,
                            "payload": payload,
                            "type": "Code Injection"
        ...
        if not vulnerable:
            results["result"]["message"] = "Code Injection не найдены"
