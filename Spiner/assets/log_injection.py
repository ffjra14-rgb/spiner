#!/usr/bin/env python3
# log_injection.py - Log Injection Scanner
...
PAYLOAD_FILE = "assets/payloads/Log Injection.txt"
TIMEOUT = 10
...
def load_payloads() -> List[str]:
    try:
        ...
        return ["admin%0a[INFO] User logged in"]
...
def run(target: str, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        for param in params:
            for payload in payloads[:30]:
                ...
                try:
                    ...
                    # Проверяем, не появились ли признаки логирования в ответе
                    if "[INFO]" in resp.text or "[ERROR]" in resp.text or "[WARNING]" in resp.text:
                        vulnerable.append({
                            "param": param,
                            "payload": payload,
                            "type": "Log Injection"
        ...
        if not vulnerable:
            results["result"]["message"] = "Log инъекции не найдены"
