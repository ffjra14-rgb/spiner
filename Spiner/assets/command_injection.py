#!/usr/bin/env python3
# command_injection.py - Command Injection Scanner
...
PAYLOAD_FILE = "assets/payloads/Command Injection.txt"
TIMEOUT = 10
...
def detect_command_execution(response_text: str) -> bool:
...
def run(target: str, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        for param in params:
            for payload in payloads[:50]:
                ...
                try:
                    ...
                    if detect_command_execution(resp.text):
                        vulnerable.append({
                            "param": param,
                            "payload": payload,
        ...
        if not vulnerable:
            results["result"]["message"] = "Command Injection не найдены"
